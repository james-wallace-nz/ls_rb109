...
=begin
The above code is not performing as expected because it is using integer division
on line 26 instead of float division. By changing one of the data types to float
in the division operation, we can cause our result to be that of float division
rather than integer.

Further, there is no code to keep the program from breaking if the number of
documents with the current term is 0 (causing a ZeroDivisionError). TODO figure
out how to fix this. What is the idf if the number of docs with the given term
is 0? Make that the result for this edge case.
=end